    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto">
                <li class="nav-item">
                    <a class="nav-link ${activePage eq 'index' ? 'active' : ''}"
                       href="${pageContext.request.contextPath}/index">Home</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link ${activePage eq 'search' ? 'active' : ''}"
            ...
            <c:if test="${!(param.searchBox eq 'noShow')}">
                <form action="${pageContext.request.contextPath}/search" method="get" class="input-group">
                    <input type="text" class="form-control" name="query" placeholder="Search music..."
                           value="${param.query}">
            ...
            <div class="navbar-actions">
                <c:choose>
                    ...
                    <c:otherwise>
                        <button type="button" class="btn cart-btn-nav" data-bs-toggle="modal"
                                data-bs-target="#shoppingCartModal">
                            <i class="fas fa-shopping-cart"></i>
                            <c:set var="cartItemCount"
                                   value="${sessionScope.cartItems != null ? sessionScope.cartItems.size() : 0}"/>
                            <span class="cart-badge"
                                  style="display: ${cartItemCount > 0 ? 'flex' : 'none'};">${cartItemCount}</span>
                        </button>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:choose>
                                    <c:when test="${sessionScope.USER.userType.name() eq 'ARTIST'}">
                                        <a href="${pageContext.request.contextPath}/artist/profile"
                                           class="btn btn-outline-light btn-sm">
                                            <i class="fa-solid fa-user me-1"></i>Artist Profile
                                        </a>
                                    </c:when>
                                    <c:otherwise>
