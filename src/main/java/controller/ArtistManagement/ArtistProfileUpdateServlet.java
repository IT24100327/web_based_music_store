    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (userId != currentArtist.getUserId()) {
                response.sendRedirect(request.getContextPath() + "/artist/profile?error=You can only update your own profile.");
                return;
            }
            ...
            // 3. Update the session with the new artist details
            session.setAttribute("USER", artistToUpdate);

            // 4. Redirect back to the profile page with a success message
            response.sendRedirect(request.getContextPath() + "/artist/profile?success=Profile updated successfully!");

        } catch (NumberFormatException e) {
            response.sendRedirect(request.getContextPath() + "/artist/profile?error=Invalid User ID format.");
        } catch (IllegalArgumentException | SQLException e) {
            e.printStackTrace(); // Log the full error for debugging.
            response.sendRedirect(request.getContextPath() + "/artist/profile?error=Update failed: " + e.getMessage());
        }
    }
}